...
export const Header = ({ isHeaderVisible }: Header) => {
  const [isDialogOpen, setIsDialogOpen] = useState(false);

  return (
    <motion.nav
      className="fixed top-0 left-0 w-full z-40 backdrop-blur-md border-b border-gray-200"
      initial={{ y: 0 }}
        ...
        <div className="flex space-x-8">
          <NavigationMenu>
            <NavigationMenuList className="hidden md:flex gap-8">
              {["home", "about", "experience", "projects", "blogs"].map(
                (section) => (
                  <NavigationMenuItem key={section}>
                    <NavigationMenuLink
                      href={`/#${section}`}
                      className="flex items-center gap-2 transition-colors font-medium relative py-2 text-gray-500 hover:text-gray-800"
                    >
